...
class LacasaPlugin(val global: Global) extends NscPlugin { self =>
...
//  val boxOpenMethod = boxClass.tpe.member(newTermName("open"))
  ...
  object PluginComponent extends NscPluginComponent {
    val global = LacasaPlugin.this.global
    import global.{log => _, _}
    import definitions._
    import reflect.internal.Flags._

    // NEWNEWNEW
    val blockTypeConstructor = global.weakTypeOf[blocks.OcapBlock[Int,Int]]
    ...
    // dependencies are classes whose status has not been determined
    //
    ...
    def mkInsecure(cls: Symbol): Unit = {
      // we can remove dependencies as soon as a class is insecure since at that
      // point its status cannot change
      if (deps.isDefinedAt(cls)) // remove dependencies
        deps = deps - cls
      if (depsGlobal.isDefinedAt(cls)) // remove global dependencies
        depsGlobal = depsGlobal - cls

      addInsecureClass(cls) // add to insecure set
    ...
    // NEWNEWNEW
    // Traverser for the blocks. We check for ocap constraint, this is the same
    // checks as for the openBox: all accessed objects must be safe and all instanciated
    // classes must be ocap
    class BlockClassTraverser( unit: CompilationUnit ) extends Traverser {
      override def traverse( tree: Tree ): Unit = tree match {
        
        case DefDef( mods, tname, tparams, vparamss, tpt, rhs ) =>


          if (tname == TermName("apply")) {
    ...
    class OcapStrictTraverser(unit: CompilationUnit) extends Traverser {
      ...
      override def traverse(tree: Tree): Unit = tree match {
        ...
        // NEWNEWNEW
        // this case is added from original lacasa plugin
        case cl @ ClassDef(mods, tname, tparams, impl)  =>
          println(cl)
          println(cl.symbol.toType)
          println(cl.symbol.toType <:< blockTypeConstructor)
          println(blockTypeConstructor)
          if (cl.symbol.toType <:< blockTypeConstructor) {
            println("checking block")
            // Look for ocap leaks
            val bct = new BlockClassTraverser(unit)
            bct.traverse(impl) 

          }
          // traverse the tree normally in order to find class defs and stuff that is within
          super.traverse(tree) 
...
// This was the ocap check for lacasa, we do not apply it here
//
//        case Apply(fun, args) if fun.symbol == boxOpenMethod =>
//          // In case of applyign boxOpen we can not access 
//          val leakChecker = new Traverser {
//            override def traverse(tree: Tree): Unit = tree match {
//              case sel @ Select(obj, _) =>
//                val potentialMod = sel.symbol.owner
